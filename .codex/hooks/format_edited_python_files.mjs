...
const pythonFileExtensions = new Set([".py", ".pyi"]);
...
const isPythonFile = (file) => pythonFileExtensions.has(extname(file.relativePath));

const getBlackInvocation = (repositoryRoot) => {
  const localBlack = join(repositoryRoot, ".venv", "bin", "black");
  const localWindowsBlack = join(repositoryRoot, ".venv", "Scripts", "black.exe");

  if (existsSync(localBlack)) {
    return { command: localBlack, args: ["--quiet"] };
  }

  if (existsSync(localWindowsBlack)) {
    return { command: localWindowsBlack, args: ["--quiet"] };
  }

  return { command: "uv", args: ["run", "--no-sync", "black", "--quiet"] };
};
...
const writeContext = (relativePaths, outsideRepositoryPaths) => {
  ...
  if (relativePaths.length > 0) {
    messages.push(`Black processed edited Python files: ${relativePaths.join(", ")}`);
  }
...
const main = async () => {
  ...
  if (files.length > 0) {
    const black = getBlackInvocation(repositoryRoot);
    const result = spawnSync(
      black.command,
      [...black.args, ...files.map((file) => file.absolutePath)],
      {
        cwd: repositoryRoot,
        encoding: "utf8",
      },
    ...
    if (result.status !== 0) {
      const detail = result.error?.message ?? result.stderr.trim() ?? "Unknown Black error";
      throw new Error(`Unable to format edited Python files: ${detail}`);
    }
  }
